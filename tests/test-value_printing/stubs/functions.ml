...
struct
  ...
  (* let min_name = match sizeof (ptr void) with *)
  (*   | 4 -> "retrieve_INT32_MIN" *)
  (*   | 8 -> "retrieve_INT64_MIN" *)
  (*   | _ -> assert false in *)
  (* let retrieve_nINT_MIN = Foreign.foreign min_name ~from:testlib *)
  (*     (void @-> returning nativeint) in *)

  (* let max_name = match sizeof (ptr void) with *)
  (*   | 4 -> "retrieve_INT32_MAX" *)
  (*   | 8 -> "retrieve_INT64_MAX" *)
  (*   | _ -> assert false in *)
