...
and format_structured : type a b. Format.formatter -> (a, b) structured -> unit
  = fun fmt ({structured = CPointer {Ctypes_ptr.reftype}} as s) ->
    let open Format in
    match reftype with
    | Struct {fields} ->
      fprintf fmt "{@;<1 2>@[";
...
and format_array : type a. Format.formatter -> a carray -> unit
  = fun fmt ({astart = CPointer {Ctypes_ptr.reftype}; alength} as arr) ->
    let open Format in
    fprintf fmt "{@;<1 2>@[";
...
and format_ptr : type a. Format.formatter -> a ptr -> unit
  = fun fmt (CPointer {Ctypes_ptr.raw_ptr; reftype}) ->
