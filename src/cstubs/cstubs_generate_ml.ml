...
let primname : prefix:string -> string -> string =
  fun ~prefix name -> Printf.sprintf "%s_%s" prefix name
...
let fn : type a b. stub_name:string -> external_name:string -> (a -> b) fn -> extern * ml_dec option =
   fun ~stub_name ~external_name fn -> 
     ...
     ({ ident = external_name;
        typ = typ;
        primname = stub_name (* primname ~prefix name *);
        primname_byte = primname_byte stub_name typ; (* TODO *)
